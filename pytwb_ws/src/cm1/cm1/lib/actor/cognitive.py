import sys
from math import atan2, degrees, isinf, radians, sqrt

import cv2
import numpy as np
import py_trees
import pyrealsense2 as rs
from lib.pointlib import PointBag, PointEx
...
class CognitiveNetwork(SubNet):
    ...
    def read_marker(self):
        ...
        try:
            parameters = cv2.aruco.DetectorParameters_create()  # OpenCV < 4.8
        except AttributeError:
            parameters = cv2.aruco.DetectorParameters()  # OpenCV >= 4.8

        _, ids, _ = cv2.aruco.detectMarkers(
            input_img, dictionary, parameters=parameters
        )
        # print(ids)
        return ids
