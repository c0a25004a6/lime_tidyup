...
def parse_candidates(path: Path) -> dict[str, object]:
    ...
    for index, row in enumerate(rows):
        ...
        require(min(
            row["left_x_overlap_m"], row["left_z_overlap_m"],
            row["right_x_overlap_m"], row["right_z_overlap_m"],
        ) > 0.0, "candidate lost positive XZ overlap")
        require(row["common_low_m"] <= row["common_high_m"] + 1e-12, "candidate lost mimic reach interval")
    return {"meta": meta, "rows": rows}


def parse_evaluation(path: Path) -> dict[str, object]:
    meta = None
    baseline = None
    candidates = []
    selected = None
    for raw in path.read_text().splitlines():
        ...
        fields = raw.split("\t")
        if fields[0] == "META":
            require(len(fields) == 7 and fields[1] == "1", "invalid evaluator META")
            meta = {
                "robot_name": fields[2],
                "state_count": int(fields[3]),
                "candidate_count": int(fields[4]),
                "anchor_index": int(fields[5]),
            }
        elif fields[0] == "BASELINE":
            require(len(fields) == 5, "invalid BASELINE row")
            baseline = {
                "clear": fields[1] == "true",
                "first_collision_index": int(fields[2]),
                "pairs": fields[3].split(",") if fields[3] else [],
            }
        elif fields[0] == "CANDIDATE":
            require(len(fields) == 14, "invalid evaluator CANDIDATE row")
            candidates.append({
                "rank": int(fields[1]),
                "translation_m": [float(value) for value in fields[2:5]],
                "norm_m": float(fields[5]),
            ...
            require(len(fields) == 9, "invalid SELECT row")
            selected = {
                "found": fields[1] == "true",
                "translation_m": [float(value) for value in fields[2:5]],
                "norm_m": float(fields[5]),
    ...
    require(meta is not None and baseline is not None and selected is not None, "evaluator file incomplete")
    require(meta["robot_name"] == "turtlebot3_lime", "robot model mismatch")
    ...
    require(baseline["clear"] is False and baseline["first_collision_index"] == 0, "baseline blocker not reproduced")
    require(sorted(baseline["pairs"]) == expected_pairs, "baseline contact pairs mismatch")
    if selected["found"]:
        require(selected["selected_states_checked"] == STATE_COUNT, "selected candidate did not traverse full path")
        require(candidates and candidates[-1]["clear"] is True, "selected clear row missing")
        require(candidates[-1]["states_checked"] == STATE_COUNT, "selected evaluator state count mismatch")
        require(candidates[-1]["translation_m"] == selected["translation_m"], "selected translation mismatch")
    return {"meta": meta, "baseline": baseline, "candidates": candidates, "selected": selected}
...
def main() -> int:
    ...
    require(pr23.get("source_ref") == PR23_SOURCE and pr23.get("audit_passed") is True, "PR23 evidence mismatch")
    require(pr23.get("decision") == "FULL_PATH_CLEAR_TRANSLATION_FOUND", "PR23 decision mismatch")
    require(pr24.get("source_ref") == PR24_SOURCE and pr24.get("audit_passed") is True, "PR24 evidence mismatch")
    require(pr24.get("decision") == "BLOCKED_CORRECTED_FIXTURE_OUTSIDE_FINGER_REACH_ENVELOPE", "PR24 blocker mismatch")

    coarse_candidates = parse_candidates(Path(args.coarse_candidates))
    coarse = parse_evaluation(Path(args.coarse_results))
    require(coarse["meta"]["candidate_count"] == coarse_candidates["meta"]["relevant"], "coarse candidate coverage mismatch")
    ...
    if coarse["selected"]["found"]:
        ...
        fine = parse_evaluation(Path(args.fine_results))
        require(fine["meta"]["candidate_count"] == fine_candidates["meta"]["relevant"], "fine candidate coverage mismatch")

    found = bool(fine and fine["selected"]["found"])
    selected = fine["selected"] if found else (coarse["selected"] if coarse["selected"]["found"] else None)
    if found:
    ...
    summary = {
        ...
        "selected_candidate": selected if found else None,
        "coarse": {
            "generator": coarse_candidates["meta"],
            "evaluated_before_stop": len(coarse["candidates"]),
            "selected": coarse["selected"],
        },
        "fine": None if fine is None else {
            "generator": fine_candidates["meta"],
            "evaluated_before_stop": len(fine["candidates"]),
            "selected": fine["selected"],
        },
        "full_path_state_count": STATE_COUNT,
        "relevance_constraints": {
            "positive_left_right_xz_aabb_overlap_required": True,
            "common_exact_limit_mimic_side_plane_interval_required": True,
            "cube_support_rigid_transform_preserved": True,
        },
        "next_required_phase": next_phase,
        "claim_limits": {
            "grid_search_only": True,
            "continuous_space_optimum_claimed": False,
            "continuous_between_sample_clearance_claimed": False,
            "exact_mesh_contact_claimed": False,
    ...
    inputs = {
        "pr23_summary": sha256(Path(args.pr23_summary)),
        "pr24_summary": sha256(Path(args.pr24_summary)),
        "coarse_candidates": sha256(Path(args.coarse_candidates)),
        "coarse_results": sha256(Path(args.coarse_results)),
    }
    ...
    manifest = {
        ...
        "decision": decision,
        "candidate_found": found,
        "selected_translation_m": None if not found else selected["translation_m"],
        "inputs": inputs,
