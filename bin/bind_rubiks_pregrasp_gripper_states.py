#!/usr/bin/env python3
"""Bind measured gripper JointState values to exact ground-audit root/arm states."""
...
def urdf_contract(path: Path) -> dict[str, Any]:
    ...
    for control in root.findall("ros2_control"):
        for joint in control.findall("joint"):
            ...
            controls[name] = {
                ...
                "command": [parse_interface(item) for item in joint.findall("command_interface")],
                "state": [parse_interface(item) for item in joint.findall("state_interface")],
            }
    ...
    for name in GRIPPER_JOINTS:
        ...
        require(parent is not None and parent.get("link") == "link7", f"{name} parent mismatch")
        require(child is not None and child.get("link"), f"{name} child missing")
        require(limit is not None and limit.get("lower") and limit.get("upper"), f"{name} limits missing")
        urdf_lower = float(limit.get("lower"))
        urdf_upper = float(limit.get("upper"))
        axis_values = [float(value) for value in (axis.get("xyz") if axis is not None else "").split()]
        require(len(axis_values) == 3 and all(math.isfinite(value) for value in axis_values), f"{name} axis invalid")

        control = controls.get(name)
        require(control is not None, f"{name} ros2_control declaration missing")
        command = next((item for item in control["command"] if item["name"] == "position"), None)
        ...
        require(isinstance(control_lower, float) and isinstance(control_upper, float), f"{name} control limits missing")
        state_names = [item["name"] for item in control["state"]]
        require("position" in state_names and "velocity" in state_names, f"{name} state interfaces incomplete")
    ...
    require(right["mimic"] == {"joint": "gripper_left_joint", "multiplier": 1.0, "offset": 0.0}, "right mimic contract mismatch")
    require(right["ros2_control_parameters"].get("mimic") == "gripper_left_joint", "right ros2_control mimic source mismatch")
    require(float(right["ros2_control_parameters"].get("multiplier", "nan")) == 1.0, "right ros2_control mimic multiplier mismatch")
    ...
    for name in GRIPPER_LINKS:
        ...
        if not collisions:
            ...
            continue
        data = b"".join(ET.tostring(element, encoding="utf-8") for element in collisions)
...
def read_ground_states(path: Path) -> list[dict[str, Any]]:
    ...
    for raw in path.read_text().splitlines():
        ...
        require(values.shape == (13,) and np.isfinite(values).all(), f"invalid ground-state values: {fields[0]}")
        states.append({"label": fields[0], "values": values})
    require(states, "ground-state matrix is empty")
    return states


def match_samples(simulation: dict[str, Any], trace: dict[str, Any], ground_states: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    simulation_samples = simulation.get("evidence", {}).get("joint_state_samples", [])
    observer_samples = trace.get("samples", [])
    require(len(simulation_samples) == len(ground_states) and len(simulation_samples) >= 100, "upstream state count mismatch")
    ...
    for index, (simulation_sample, ground_state) in enumerate(zip(simulation_samples, ground_states)):
        require(ground_state["label"] == f"sample_{index:06d}", f"ground-state label mismatch at {index}")
        stamp = int(simulation_sample.get("ros_stamp_ns", -1))
        arm = np.asarray(simulation_sample.get("positions_rad"), dtype=float)
        require(arm.shape == (6,) and np.isfinite(arm).all(), f"invalid simulation arm sample {index}")
        require(np.max(np.abs(ground_state["values"][7:] - arm)) <= MAX_ARM_MATCH_ERROR_RAD, f"ground/arm mismatch at {index}")
        candidates = []
        for candidate in by_stamp.get(stamp, []):
            ...
            if observed_arm.shape == (6,) and np.isfinite(observed_arm).all():
                candidates.append((float(np.max(np.abs(observed_arm - arm))), candidate))
        ...
        require(arm_error <= MAX_ARM_MATCH_ERROR_RAD, f"arm match error at {index}: {arm_error}")
        used.add(id(candidate))
        arm_errors.append(arm_error)

        gripper = np.asarray(candidate.get("gripper_positions_m"), dtype=float)
        require(gripper.shape == (2,) and np.isfinite(gripper).all(), f"invalid gripper positions at {index}")
        velocity = candidate.get("gripper_velocities_m_s")
        if velocity is None:
            missing_velocity_count += 1
            gripper_velocity = np.asarray([math.nan, math.nan])
        else:
            gripper_velocity = np.asarray(velocity, dtype=float)
            require(gripper_velocity.shape == (2,) and np.isfinite(gripper_velocity).all(), f"invalid gripper velocity at {index}")
        left, right = float(gripper[0]), float(gripper[1])
        left_values.append(left)
        right_values.append(right)
        mimic_positions.append(abs(right - left))
        if velocity is not None:
            left_velocities.append(float(gripper_velocity[0]))
            right_velocities.append(float(gripper_velocity[1]))
            mimic_velocities.append(abs(float(gripper_velocity[1] - gripper_velocity[0])))
        rows.append({
            "label": ground_state["label"],
            "ground_values": ground_state["values"].tolist(),
            "gripper_positions_m": [left, right],
        })

    require(missing_velocity_count == 0, f"{missing_velocity_count} matched samples lack gripper velocity")
    require(max(mimic_positions) <= MAX_MIMIC_POSITION_RESIDUAL_M, "gripper mimic position residual exceeded")
    require(max(mimic_velocities) <= MAX_MIMIC_VELOCITY_RESIDUAL_M_S, "gripper mimic velocity residual exceeded")
    return rows, {
        ...
        "right_position_min_m": min(right_values),
        "right_position_max_m": max(right_values),
        "right_position_range_m": max(right_values) - min(right_values),
        "maximum_abs_left_velocity_m_s": max(abs(value) for value in left_velocities),
        "maximum_abs_right_velocity_m_s": max(abs(value) for value in right_velocities),
        "maximum_mimic_position_residual_m": max(mimic_positions),
        "maximum_mimic_velocity_residual_m_s": max(mimic_velocities),
        "maximum_allowed_mimic_position_residual_m": MAX_MIMIC_POSITION_RESIDUAL_M,
        "maximum_allowed_mimic_velocity_residual_m_s": MAX_MIMIC_VELOCITY_RESIDUAL_M_S,
    }
...
def write_states(path: Path, rows: list[dict[str, Any]]) -> None:
    with path.open("w", encoding="utf-8") as stream:
        for row in rows:
            ...
            stream.write(row["label"] + "\t" + "\t".join(f"{value:.17g}" for value in values) + "\n")
...
def main() -> int:
    ...
    require(simulation.get("source_ref") == args.source_ref, "simulation source_ref mismatch")
    require(trace.get("source_ref") == args.source_ref, "gripper trace source_ref mismatch")
    require(simulation.get("passed") is True and trace.get("passed") is True, "upstream evidence failed")
    ...
    require(trace.get("gripper_joint_order") == GRIPPER_JOINTS, "observer gripper order mismatch")
    require(trace.get("errors") == [], "gripper observer contains errors")
    safety = trace.get("safety", {})
    require(safety.get("read_only_subscription_only") is True, "gripper observer is not read-only")
    for key in ("publisher_created", "service_client_created", "action_client_created", "controller_loaded", "command_sent"):
        require(safety.get(key) is False, f"observer {key} must be false")
    simulation_safety = simulation.get("safety", {})
    require(simulation_safety.get("gripper_controller_loaded") is False, "gripper controller was loaded")
    require(simulation_safety.get("gripper_command_sent") is False, "gripper command was sent")

    simulation_contract = urdf_contract(Path(args.simulation_urdf))
    moveit_contract = urdf_contract(Path(args.moveit_urdf))
    require(simulation_contract["robot_name"] == "turtlebot3_lime_arm_motion_test", "simulation URDF name mismatch")
    require(moveit_contract["robot_name"] == "turtlebot3_lime", "MoveIt URDF name mismatch")
    require(not simulation_contract["missing_collision_geometry"], "simulation gripper geometry missing")
    require(not moveit_contract["missing_collision_geometry"], "MoveIt gripper geometry missing")
    require(simulation_contract["collision_geometry_sha256"] == moveit_contract["collision_geometry_sha256"], "gripper collision geometry mismatch")
    require(simulation_contract["joints"] == moveit_contract["joints"], "gripper joint contract mismatch")
    ...
    for row in rows:
        for name, value in zip(GRIPPER_JOINTS, row["gripper_positions_m"]):
            ...
            require(contract["effective_lower_m"] <= value <= contract["effective_upper_m"], f"{name} effective limit violation")
