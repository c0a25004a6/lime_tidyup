...
struct AuditState
{
  ...
  double left_gripper_position;
  double right_gripper_position;
};
...
std::vector<AuditState> read_states(const std::string& path)
{
  ...
  while (std::getline(stream, line))
  {
    ...
    if (fields.size() != 16)
    {
      throw std::runtime_error("invalid gripper-ground state field count for " + line);
    }
    AuditState state;
    ...
    state.arm_positions.assign(values.begin() + 7, values.begin() + 13);
    state.left_gripper_position = values[13];
    state.right_gripper_position = values[14];
    states.push_back(std::move(state));
  }
  if (states.empty())
  {
    throw std::runtime_error("gripper-ground state matrix is empty");
  }
  return states;
}
  ...
  try
  {
    if (argc != 5)
    {
      throw std::runtime_error(
        "usage: rubiks_gripper_ground_clearance_preflight URDF SRDF STATES_TSV OUTPUT_TSV");
    }
    ...
    const auto active_arm_joints = arm_group->getActiveJointModelNames();
    if (active_arm_joints != expected_arm_joints)
    {
      throw std::runtime_error("active arm joint order mismatch: " + join(active_arm_joints, ","));
    }
    ...
    if (robot_model->getJointModel("gripper_left_joint") == nullptr ||
        robot_model->getJointModel("gripper_right_joint") == nullptr)
    {
      throw std::runtime_error("gripper joint model is missing");
    }
